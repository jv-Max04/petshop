...
app.delete("/servico/:id", (req, res) => {
  ...
  con.query(
    `delete from servico where id_servico = ${req.params.id}`,
    (erro, resp) => {
      checkErr(res, erro, resp);
    }
  );
...
function checkErrComID(res, err, resp) {
  ...
  else if (resp) res.status(201).json(resp);
  else res.status(400).json({ message: "ID de Satisfação não encontrado" });
}
...
app.post("/satisfacao", (req, res) => {
  const body = req.body;
  const sql =
    "INSERT INTO satisfacao (id_cliente, nota, feedback) VALUES (?, ?, ?)";
...
app.put("/satisfacao/:id", (req, res) => {
  ...
  con.query(
    `UPDATE satisfacao SET ? WHERE id_pesquisa = ${req.params.id}`,
    body,
    (err, resp) => {
...
app.delete("/satisfacao/:id", (req, res) => {
  con.query(
    `DELETE FROM satisfacao WHERE id_pesquisa = ${req.params.id}`,
    (err, resp) => {
      checkErrComID(res, err, resp);
    }
  );
...
app.put("/animal/:id", (req, resp) => {
  ...
  con.query(
    `update animal set ? where id_animal = ${id}`,
    corpo,
    (erro, resposta) => {
      if (erro) {
        resp.json(erro);
...
app.put("/dono/:id", (req, res) => {
  ...
  con.query(
    `update dono set ? where id_cliente=${req.params.id}`,
    atdono,
    (erro, resposta) => {
      checkErr(res, erro, resposta);
    }
...
app.post("/funcionario/", (req, res) => {
  ...
  const inserir =
    "INSERT INTO funcionario(nome, cpf, salario, funcao) VALUES(?, ?, ?, ?);";
  con.query(
    inserir,
    [
...
app.put("/atendimento/:id", (req, res) => {
  ...
  con.query(
    "update atendimento set ? where id_atendimento = " + id,
    body,
    (erro, resp) => {
      checkErr(res, erro, resp);
    }
...
app.delete("/atendimento/:id", (req, res) => {
  ...
  con.query(
    `delete from atendimento where id_atendimento = ${req.params.id} `,
    (erro, resp) => {
      checkErr(res, erro, resp);
    }
  );
